...
Y ya está: `http://localhost:3000` pide crear una cuenta y entra. **No hace falta instalar Postgres.** Sin `DATABASE_URL`, la app levanta [PGlite](https://pglite.dev), que es Postgres compilado a WebAssembly, y guarda la base en `.datos/dev`. Las migraciones se aplican solas al arrancar.
...
| Variable | Para qué |
| --- | --- |
| `DATABASE_URL` | El Postgres de producción. Si está, se usa; si no, PGlite. Con Supabase o Neon, la cadena del *pooler*. |
