...
(function () {
  ...
  const almacen = (() => {
    ...
    async function refrescar() {
      if (!conCuenta || enviando || !confirmado) return;
  ...
  const nombreMesCorto = (mesClave) => {
    const { year, month } = M.splitMonthKey(mesClave);
  ...
  function bloqueDeudasCorto(cuentas) {
    const filas = datos.personas
      .map((persona) => ({
        persona,
        cents: montosPersonaFiltrados(
          cuentas.byPerson.find((p) => p.personId === persona.id),
          null,
          mes,
          null,
        ).cobrar,
  ...
  function filtrosGastos() {
    ...
    if (categorias.length === 0 && medios.length === 0) return '';
  ...
  function cabezaGastos(rotuloExtra, filtrando) {
    ...
    return `
        <div class="bloque__cabeza${filtros ? ' bloque__cabeza--apilada' : ''}">
          <div class="bloque__cabeza__fila">
            <div>
  ...
  function itemCobrarFiltrado(item, mesFiltro, catFiltro) {
    ...
    if (catFiltro && item.categoryId !== catFiltro) return false;
    return true;
  }

  function itemCobrarCoincideFiltro(item, mesFiltro, catFiltro) {
    if (mesFiltro && M.monthKeyOf(item.occurredAt, OFFSET) !== mesFiltro) return false;
    if (catFiltro && item.categoryId !== catFiltro) return false;
    return true;
  }

  function itemPagarFiltrado(deuda, mesFiltro, catFiltro) {
    if (deuda.settledAt) return false;
    if (mesFiltro && M.monthKeyOf(deuda.occurredAt, OFFSET) !== mesFiltro) return false;
    if (catFiltro && categoriaDeDeuda(deuda) !== catFiltro) return false;
    return true;
  }

  function itemPagarCoincideFiltro(deuda, mesFiltro, catFiltro) {
    if (mesFiltro && M.monthKeyOf(deuda.occurredAt, OFFSET) !== mesFiltro) return false;
    if (catFiltro && categoriaDeDeuda(deuda) !== catFiltro) return false;
    return true;
  }

  function montoItemCobrar(item) {
    return item.pendingCents > 0 ? item.pendingCents : item.amountCents;
  }

  function itemCobrarDelMes(item, mesFiltro) {
    return itemCobrarCoincideFiltro(item, mesFiltro, null) && montoItemCobrar(item) > 0;
  }

  function itemPagarDelMes(deuda, mesFiltro) {
    return itemPagarCoincideFiltro(deuda, mesFiltro, null) && deuda.amountCents > 0;
  }

  function itemsCobrarDelMes(cuenta, mesFiltro) {
    return (cuenta?.items ?? [])
      .filter((item) => itemCobrarDelMes(item, mesFiltro))
      .sort((a, b) => b.occurredAt.localeCompare(a.occurredAt));
  }

  function itemsPagarDelMes(mio, mesFiltro) {
    return (mio?.items ?? [])
      .filter((deuda) => itemPagarDelMes(deuda, mesFiltro))
      .sort((a, b) => b.occurredAt.localeCompare(a.occurredAt));
  }

  function pendientesCobrarDePersona(cuenta, mesFiltro) {
    return (cuenta?.items ?? []).filter((item) => itemCobrarFiltrado(item, mesFiltro, null));
  }

  function pendientesPagarDePersona(mio, mesFiltro) {
    return (mio?.items ?? []).filter((deuda) => itemPagarFiltrado(deuda, mesFiltro, null));
  }

  function montosPersonaFiltrados(cuenta, mio, mesFiltro, catFiltro) {
    const cobrar = (cuenta?.items ?? [])
      .filter((item) => itemCobrarFiltrado(item, mesFiltro, catFiltro))
      .reduce((s, item) => s + item.pendingCents, 0);
    const pagar = (mio?.items ?? [])
      .filter((deuda) => itemPagarFiltrado(deuda, mesFiltro, catFiltro))
      .reduce((s, deuda) => s + deuda.amountCents, 0);
  ...
  function mesesParaFiltroPersonas() {
    ...
    for (const reparto of datos.repartos) {
      const gasto = datos.gastos.find((g) => g.id === reparto.expenseId && !g.deletedAt);
      if (!gasto || gasto.status !== 'confirmed') continue;
      const pagado = datos.asignaciones
        .filter((a) => a.splitId === reparto.id)
        .reduce((s, a) => s + a.amountCents, 0);
      if (reparto.amountCents - pagado <= 0) continue;
      meses.add(M.monthKeyOf(gasto.occurredAt, OFFSET));
    }
    for (const deuda of datos.deudas) {
      if (!deuda.settledAt) meses.add(M.monthKeyOf(deuda.occurredAt, OFFSET));
    }
    ...
    return `
        <div class="personas-filtros__grupo">
          ...
          <div class="segmentos segmentos--bloque personas-filtros__segmentos" role="group" aria-label="Filtrar por mes">
            <button type="button" class="segmento" data-filtro-personas-mes="todo"
                    aria-pressed="${filtroPersonasMes === null ? 'true' : 'false'}">
              Todos
            </button>
            ${meses
              .map(
                (mesClave) => `
              <button type="button" class="segmento" data-filtro-personas-mes="${mesClave}"
                      aria-pressed="${filtroPersonasMes === mesClave ? 'true' : 'false'}">
                ${escapar(nombreMesCorto(mesClave))}
              </button>`,
              )
              .join('')}
          </div>
  ...
  function resumenLineaPersona(cobrar, pagar, filtrado) {
    const partes = [];
    ...
    return `
      <button type="button" class="persona-fila" data-ver-persona="${persona.id}">
        ...
        <span class="persona-fila__medio">
          ...
          <span class="persona-fila__resumen">${resumenLineaPersona(cobrar, pagar, filtrado)}</span>
        </span>
        ${
          montoDerecha
            ? `<span class="persona-fila__neto cifra" data-signo="${signoNeto}">${montoDerecha}</span>`
            : `<span class="persona-fila__neto persona-fila__neto--cero">${filtrado ? '—' : 'Al día'}</span>`
        }
  ...
  function totalesPorCategoriaPlano(items, categoriaDe, montoDe) {
    const map = new Map();
  ...
  function itemsCobrarPendientes(cuenta, mesFiltro) {
    return (cuenta?.items ?? []).filter(
      (item) =>
        !item.isSettled &&
  ...
  function itemsPagarPendientes(mio, mesFiltro) {
    return (mio?.items ?? []).filter(
      (deuda) => !deuda.settledAt && deuda.amountCents > 0 && itemPagarCoincideFiltro(deuda, mesFiltro, null),
    );
  }
  ...
  function pistaDesglosePersona(lado) {
    ...
    const mesEtiqueta = personaDetalleDesglose.mes
      ? nombreMes(personaDetalleDesglose.mes)
      : filtroPersonasMes
        ? nombreMes(filtroPersonasMes)
        : null;
  ...
  function filtrarItemsDesglosePersona(items, lado, fechaDe, categoriaDe) {
    if (!personaDetalleDesglose || personaDetalleDesglose.lado !== lado) return items;
    const { mes, categoria } = personaDetalleDesglose;
    return items.filter((item) => {
      const mesItem = M.monthKeyOf(fechaDe(item), OFFSET);
      if (mes) {
        if (mesItem !== mes) return false;
      } else if (filtroPersonasMes && mesItem !== filtroPersonasMes) {
        return false;
      }
      return (categoriaDe(item) ?? 'sin-categoria') === categoria;
    });
  }
  ...
  function detallePersona(persona, cuentas, mias) {
    ...
    const itemsCobrarVisibles = filtradoMes
      ? itemsCobrarDelMes(cuenta, mesFiltro)
      : [
    ...
    const itemsPagarVisibles = filtradoMes
      ? itemsPagarDelMes(mio, mesFiltro)
      : [
    ...
    const itemsCobrarResumen = filtradoMes
      ? itemsCobrarDelMes(cuenta, mesFiltro)
      : itemsCobrarPendientes(cuenta, mesFiltro);
    const itemsPagarResumen = filtradoMes
      ? itemsPagarDelMes(mio, mesFiltro)
      : itemsPagarPendientes(mio, mesFiltro);
    const porCategoriaCobrarFlat = totalesPorCategoriaPlano(
      itemsCobrarResumen,
      (item) => item.categoryId,
      montoItemCobrar,
    );
    const porCategoriaPagarFlat = totalesPorCategoriaPlano(
      itemsPagarResumen,
      categoriaDeDeuda,
    ...
    const itemsPagarLista = filtrarItemsDesglosePersona(
      itemsPagarVisibles,
      'pagar',
      (deuda) => deuda.occurredAt,
      categoriaDeDeuda,
    );
    const desgloseCobrar = personaDetalleDesglose?.lado === 'cobrar';
    const desglosePagar = personaDetalleDesglose?.lado === 'pagar';

    const pendientesCobrar = pendientesCobrarDePersona(cuenta, filtradoMes ? mesFiltro : null);
    const pendientesPagar = pendientesPagarDePersona(mio, filtradoMes ? mesFiltro : null);
    ...
    const bloqueCobrar = hayBloqueCobrar
        ? `<div class="persona__bloque persona__bloque--cobrar">
             ...
             <div class="persona__bloque-lista">
               ${
                 itemsCobrarLista.length > 0
                   ? itemsCobrarLista.map((item) => filaDeudaCobrar(item, persona)).join('')
    ...
    const bloquePagar = hayBloquePagar
        ? `<div class="persona__bloque persona__bloque--pagar">
             ...
             <div class="persona__bloque-lista">
               ${
                 itemsPagarLista.length > 0
                   ? itemsPagarLista.map((deuda) => filaDeudaPagar(deuda)).join('')
                   : `<p class="persona__bloque-vacio">${
                       desglosePagar
                         ? 'Sin deudas en esta categoría.'
                         : filtradoMes
                           ? 'Sin deudas en este mes.'
  ...
  function cobrarTodoDePersona(idPersona) {
    ...
    const mesFiltro = filtroPersonasMes;
    const pendientes = pendientesCobrarDePersona(cuenta, mesFiltro);
    const total = pendientes.reduce((s, item) => s + item.pendingCents, 0);
    if (pendientes.length === 0 || total <= 0) return;

    const mesEtiqueta = mesFiltro ? nombreMes(mesFiltro) : null;
    const alcanceMes = mesEtiqueta ? ` de ${mesEtiqueta}` : '';
    const cantidadGastos =
      pendientes.length === 1 ? '1 gasto' : `${pendientes.length} gastos`;
    const mensaje =
      `¿Marcar que ${persona.name} te pagó todo lo pendiente${alcanceMes}?\n\n` +
      `${plata(total)} en ${cantidadGastos}.`;
    if (!confirm(mensaje)) return;

    const propuesta = M.proposeSettlementAllocation(total, pendientes);
    mutar((d) => {
      const idAbono = id();
      d.abonos.push({
        id: idAbono,
    ...
    avisar(`Listo: ${persona.name} quedó al día contigo${alcanceMes}`);
  }

  function pagarTodoDePersona(idPersona) {
    const persona = personaPorId(idPersona);
    const mio = porPagar().byPerson.find((p) => p.personId === idPersona);
    if (!persona || !mio) return;

    const mesFiltro = filtroPersonasMes;
    const pendientes = pendientesPagarDePersona(mio, mesFiltro);
    const total = pendientes.reduce((s, deuda) => s + deuda.amountCents, 0);
    if (pendientes.length === 0 || total <= 0) return;
    ...
    const cantidadGastos =
      pendientes.length === 1 ? '1 gasto' : `${pendientes.length} gastos`;
    const mensaje =
      `¿Marcar que ya pagaste todo lo que le debes a ${persona.name}${alcanceMes}?\n\n` +
      `${plata(total)} en ${cantidadGastos}.`;
    if (!confirm(mensaje)) return;
    ...
    avisar(`Listo: al día con ${persona.name}${alcanceMes}`);
  }
  ...
  function idCompartidoDeCarga(carga) {
  ...
  async function notificarAceptacionCompartido(carga) {
    if (!carga?.i || !carga?.dc || !almacen.conCuenta) return;

    try {
      await fetch('/api/compartido/aceptado', {
        method: 'POST',
        credentials: 'same-origin',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({
          repartoId: carga.i,
          emisorCorreo: carga.dc,
          montoCentavos: carga.c,
        }),
      });
    } catch (error) {
      console.warn('No se pudo avisar al emisor del enlace', error);
    }
  }
    ...
    mutar((d) => {
      ...
      for (const parte of resultado.splits) {
        ...
        d.repartos.push({
          ...
          acceptedAt: previo && mismoMonto ? (previo.acceptedAt ?? null) : null,
        });
      }
    });
  ...
  document.addEventListener('click', (evento) => {
    ...
    const filtroMesPersonas = objetivo.closest('[data-filtro-personas-mes]');
    if (filtroMesPersonas) {
    ...
    const desgloseCategoria = objetivo.closest('[data-persona-desglose-categoria]');
    if (desgloseCategoria) {
      const mes = desgloseCategoria.dataset.personaDesgloseMes ?? null;
      const categoria = desgloseCategoria.dataset.personaDesgloseCategoria;
      const lado = desgloseCategoria.dataset.personaDesgloseLado;
      if (
        personaDetalleDesglose?.mes === mes &&
        personaDetalleDesglose?.categoria === categoria &&
        personaDetalleDesglose?.lado === lado
      ) {
        personaDetalleDesglose = null;
      } else {
    ...
    if (objetivo.closest('[data-quitar-desglose-persona]')) {
      personaDetalleDesglose = null;
      pintar();
      return;
    }
