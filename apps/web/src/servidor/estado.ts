...
const pasivoMovimientoSchema = z.object({
  ...
  principalCents: opcional(centavos),
  interestCents: opcional(centavos),
  feeCents: opcional(centavos),
...
export async function leerEstado(usuarioId: string): Promise<Estado> {
  ...
  return {
    ...
    pasivos: pasivos.map((p) => ({
      ...
      currency: (p.moneda === 'USD' ? 'USD' : 'COP') as Pasivo['currency'],
      limitCents: p.cupo,
    ...
    pasivoMovimientos: pasivoMovimientos.map((m) => ({
      ...
      principalCents: m.capital,
      interestCents: m.interes,
      feeCents: m.cargos,
  ...
  return db.transaction(async (tx) => {
    ...
    await guardarFilas(
      ...
      cambios.pasivoMovimientos.puestos.map((m) => ({
        ...
        capital: m.principalCents,
        interes: m.interestCents,
        cargos: m.feeCents,
